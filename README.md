# LE Mesh Light Dimmable demo application

## Overview
This demo application shows a simple implementation of a dimmable light. The app is based on the LE Mesh Light Lightness Server model. Because the Light Lightness Server model extends the Generic OnOff and Generic Level models, the dimmable light can be controlled by a Switch (Generic OnOff Client), a Dimmer (Generic Level Client), or by an application which implements the Light Lightness Client.  The AIROC&#8482; Mesh Models library takes care of the translation of the OnOff and Level messages and the only messages that the application layer needs to process are those of the Light Lightness Model.
...
For Mesh application structure, see this [link](https://github.com/Infineon/CypressAcademy_BT101_Files/blob/master/PDFs/WBT101-07C-Mesh-Firmware.pdf)


## Features demonstrated
- LED usage on the AIROC&#8482; Kit
- Processing of the Light Lightness messages

## Instructions
To demonstrate the app, work through the following steps:

1. Build and download the application to the AIROC&#8482; Kit.
2. Build and download a controlling application such as Mesh Demo Dimmer to AIROC&#8482; Kit
3. Use MeshClient or ClientControlMesh helper applications to provision the LightDimmable node as a light bulb and the Dimmer node as a dimmer.
Note: ClientControlMesh requires an additional board that must be programmed with the mesh provision client app. Please refer to the 'AIROC-MeshClient-And-ClientControlMesh-User-Guide' to learn about these helper applications. The user guide is available in ModusToolbox&#8482; documentation or see this [link](https://www.infineon.com/dgdl/Infineon-PSoC_Programmer_Release_Notes_Version_3.28.7-Software-v01_00-EN.pdf?fileId=8ac78c8c7e7124d1017e90a4bc350c33).
4. Configure the dimmer to control the light bulb. (note that if the light bulb and the dimmer were provisioned in the same group, the dimmer will be automatically configured to send messages to the group and this step can be skipped.)
5. Push/release the  user button (SW3) on the dimmer board.  The LED (LED1 on Mesh Evaluation Kit / LED 2 on CYW920819EVB-02 Evaluation Kit) on the light bulb side should turn on.
...
- INCLUDE\_TIME\_AND\_SCHEDULER
	- Adds support for Time and Scheduler Server Models
